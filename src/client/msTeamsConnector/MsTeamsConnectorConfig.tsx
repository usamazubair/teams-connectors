...
export const MsTeamsConnectorConfig = () => {
    ...
    if (context) {
      pages.config.registerOnSaveHandler(
        (saveEvent: pages.config.SaveEvent) => {
          const settings: any = {
            entityId: colorRef.current
              ? colorRef.current.code
              : availableColors[0].code,
            contentUrl: `https://${process.env.PUBLIC_HOSTNAME}/msTeamsConnector/config.html?name={loginHint}&tenant={tid}&group={groupId}&theme={theme}`,
            configName: colorRef.current
              ? colorRef.current.title
              : availableColors[0].title,
          };
      ...
      pages.getConfig().then((config) => {
        setColor(availableColors.filter((c) => c.code === config.entityId)[0]);
      });
    }
  }, [context]);
  ...
  const colors: ShorthandCollection<DropdownItemProps> = availableColors.map(
    (clr) => {
      return {
        header: clr.title,
