 ...
 */
interface IMsTeamsConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "MsTeamsConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/msTeamsConnector/config.html")
export class MsTeamsConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as IMsTeamsConnectorData[])
                    .filter(c => {
                        return c.existing;
                    }));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as IMsTeamsConnectorData[]).map((connector, index) => {
